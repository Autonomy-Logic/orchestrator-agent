...
class ContainerNetworkManager:
    """
    Manages Docker network operations for runtime containers.
    Handles MACVLAN and internal bridge network creation and management.
    """

    def __init__(self, client: docker.DockerClient):
        """
        Initialize the ContainerNetworkManager with a Docker client.

        Args:
            client: Docker client instance for network operations
        """
        self.client = client

    def detect_interface_network(self, parent_interface: str):
        """
        ...
        This method reads from the interface cache populated by the netmon sidecar.
    ...
    def get_or_create_macvlan_network(
        ...
        if parent_subnet and parent_gateway:
            if parent_subnet.startswith("255.") or parent_subnet.count(".") == 3:
                cidr_prefix = self.netmask_to_cidr(parent_subnet)
            ...
            parent_subnet, parent_gateway = self.detect_interface_network(
                parent_interface
            )
            ...
            try:
                ...
                if "overlaps" in str(e).lower():
                    ...
                    try:
                        ...
                        log_error(
                            f"Error searching for existing networks: {search_error}"
                        )
                        raise
                else:
    ...
    def create_internal_network(self, container_name: str):
        """
